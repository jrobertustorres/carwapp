...
export class OrcamentosListPage {
  ...
  getItems(searchbar) {
    ...
    this.aux = this.aux.length == 0 ? this.orcamentoEntityResult : this.aux; 

    if(this.orcamentoEntityResult.length == 0) {
      this.orcamentoEntityResult = this.aux;
    }
  
    this.orcamentoEntityResult = this.orcamentoEntityResult.filter((v) => {
      if(v.orcamento && q) {
        // if(v.dataOrcamento && v.placaVeiculo && v.statusOrcamentoEnum && v.idOrcamentoFormat && q) {
        // if (v.servico.toLowerCase().indexOf(q.toLowerCase()) > -1) {
          if ((v.dataOrcamento.toLowerCase().indexOf(q.toLowerCase()) && v.placaVeiculo.toLowerCase().indexOf(q.toLowerCase()) && v.statusOrcamentoEnum.toLowerCase().indexOf(q.toLowerCase())  && v.idOrcamentoFormat.toLowerCase().indexOf(q.toLowerCase())) > -1) {
          return true;
        }
        return false;
      }
