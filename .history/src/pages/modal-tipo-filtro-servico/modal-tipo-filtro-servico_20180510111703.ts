...
export class ModalTipoFiltroServicoPage {
  ...
  submeterTipoFiltro() {
    ...
    if(this.filtroForm.valid) {
      ...
      this.getGpsStatus(this.filtroForm.value);
      // this.viewCtrl.dismiss({
      //   filter: this.filtroForm.value
      // });
    } else {
  ...
  submeterTipoFiltro2(formValue) {
    console.log('dentro do subemert 222222');
    this.viewCtrl.dismiss({
      filter: formValue
    });
  }

  getGpsStatus(formValue) {
  ...
  getLocation(formValue) {
    this.geolocation.getCurrentPosition().then((resp) => {
      ...
      this.submeterTipoFiltro2(formValue);
     }).catch((error) => {
