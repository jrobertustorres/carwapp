...
export class MeusDadosPage implements OnInit {
  ...
  cadastraUsuario() {
    this.dadosUsuarioForm.value.cpfPessoa = this.dadosUsuarioForm.value.cpfPessoa.replace(".", "");
    this.dadosUsuarioForm.value.cpfPessoa = this.dadosUsuarioForm.value.cpfPessoa.replace("-", "");
  ...
  editaUsuario() {
    this.usuarioDetalheEntity.cpfPessoa = this.cpfPessoa.replace("-", "");
    this.usuarioDetalheEntity.cpfPessoa = this.cpfPessoa.replace(".", "");
    console.log(this.usuarioDetalheEntity.cpfPessoa);
    // this.usuarioService
    ...
    try {
      ...
      this.usuarioService
        ...
        .then((dadosUsuarioDetalheResult) => {
          ...
          this.usuarioDetalheEntity.cpfPessoa = this.usuarioDetalheEntity.cpfPessoa.replace(/\D/g, '').replace(/(\d{2})(\d)(\d{4})(\d{4})$/, '($1) $2 $3-$4');
