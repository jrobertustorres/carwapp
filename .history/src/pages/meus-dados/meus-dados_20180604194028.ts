...
export class MeusDadosPage implements OnInit {
  ...
  getCampoTelefone(tel) {
    ...
    tel=tel.replace(/\D/g,'');
    // tel=tel.replace(/(\d{1,4})$/, '-$1');  
    // tel=tel.replace(/(\d{5,12})$/, '()$1');  
    // tel=tel.replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.'); 

    tel = tel.replace(/\([1-9]\d\)\s9?\d{4}-\d{4}/);

    // tel = tel.replace(/\D/g, '').replace(/(\d{2})(\d)(\d{4})(\d{4})$/, '($1) $2 $3-$4');
