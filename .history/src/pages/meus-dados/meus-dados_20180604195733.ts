...
export class MeusDadosPage implements OnInit {
  ...
  getCampoTelefone(tel) {
    ...
    let positionIfen = tel.length == 15 ? '/(\d{5})(\d)/' : '/(\d{4})(\d)/';
    ...
    tel=tel.replace(positionIfen,"$1-$2");    //Coloca hífen entre o quarto e o quinto dígitos
    // tel=tel.replace(/(\d{5})(\d)/,"$1-$2");    //Coloca hífen entre o quarto e o quinto dígitos
    // return v;

    this.telefonePessoa = tel;
    console.log(tel);
